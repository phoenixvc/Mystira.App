...
public class VerifyPasswordlessSigninCommandHandler
    : ICommandHandler<VerifyPasswordlessSigninCommand, (bool Success, string Message, Account? Account, string? AccessToken, string? RefreshToken)>
{
    ...
    public async Task<(bool Success, string Message, Account? Account, string? AccessToken, string? RefreshToken)> Handle(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (pendingSignin == null || !pendingSignin.IsSignin)
            {
                ...
                if (pendingByEmail != null && pendingByEmail.IsSignin && !pendingByEmail.IsUsed)
                {
                    if (pendingByEmail.FailedAttempts >= MaxFailedAttempts)
                    {
                        ...
                        return (false, "Too many failed attempts. Please request a new code.", null, null, null);
                    }

                    if (pendingByEmail.ExpiresAt < DateTime.UtcNow)
                    {
                        _logger.LogWarning("Expired signin code for email: {Email}", email);
                        return (false, "Your sign-in code has expired. Please request a new one", null, null, null);
                    }
                    ...
                    return (false, "Invalid sign-in code", null, null, null);
                }

                _logger.LogWarning("Invalid or expired signin code for email: {Email}", email);
                return (false, "Invalid or expired sign-in code", null, null, null);
            }
            ...
            if (account == null)
            {
                ...
                return (false, "Account not found", null, null, null);
            }
            ...
            return (true, "Sign-in successful", account, accessToken, refreshToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error verifying signin for email: {Email}", command.Email);
            return (false, "An error occurred while verifying your sign-in", null, null, null);
        }
    }
}
