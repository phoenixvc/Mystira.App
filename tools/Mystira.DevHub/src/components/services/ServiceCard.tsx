...
export function ServiceCard({
  ...
  const [logHeight, setLogHeight] = useState(() => {
    const saved = localStorage.getItem(`service-${config.name}-log-height`);
    return saved ? parseInt(saved, 10) : 384; // Default to 384px (max-h-96 = 24rem = 384px)
  });
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Constrain height between 100px and 80vh
      const minHeight = 100;
    ...
    const handleMouseUp = () => {
      ...
      const currentHeight = logContainerRef.current?.offsetHeight || logHeight;
      localStorage.setItem(
        `service-${config.name}-log-height`,
  ...
  }, [isResizing, config.name, logHeight]);
              ...
              {currentEnv !== "local" && environmentStatus && (
                <span
                  ...
                  title={`Environment status: ${
                    environmentStatus[currentEnv] || "unknown"
                  }`}
                >
