...
function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const getOperationStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'failed':
        return <span className="text-xs px-2 py-0.5 bg-red-100 text-red-800 rounded-full">Failed</span>;
      case 'in_progress':
        return <span className="text-xs px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-full">In Progress</span>;
      default:
        return null;
    }
  };
